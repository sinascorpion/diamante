...
class DiamanteBotClient {
  constructor(privateKey, proxy = null) {
    ...
    this.cookieString = "access_token=";
  }
  ...
  generateDeviceId() {
    ...
    return `DEV${randomHex.toUpperCase()}`;
  }
      ...
      for (const cookie of cookies) {
        if (cookie.includes('access_token=')) {
          ...
          if (match) {
            ...
            this.cookieString = `access_token=${this.accessToken}`;
          }
        }
      }
    }
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (attempt < maxRetries) {
          logger.warn(`[Attempt ${attempt}/${maxRetries}] Connection failed: ${message}, retrying in 5s...`);
          await this.sleep(5000);
          continue;
        }
        
        logger.error(`Connect wallet failed: ${message}`);
        return null;
      }
    }
    return null;
      ...
      try {
        ...
        if (response.data.success || response.data.message === "Success") {
          ...
          if (response.data.status === 429) {
            if (attempt < maxRetries) {
              logger.warn(`[Attempt ${attempt}/${maxRetries}] Faucet: ${message}, retrying in 15s...`);
  ...
  async sendTransaction(toAddress, amount) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (response.data.success || response.data.message === "Success") {
          ...
          if (response.data.status === 429 || message.includes('syncing') || message.includes('guardians') || message.includes('moment')) {
            logger.warn(`[Attempt ${attempt}/${maxRetries}] ${message}, retrying in 15s...`);
            await this.sleep(15000);
            continue;
          }
        ...
        if (status === 429 || message.includes('syncing') || message.includes('guardians') || message.includes('moment')) {
          logger.warn(`[Attempt ${attempt}/${maxRetries}] ${message}, retrying in 15s...`);
          await this.sleep(15000);
          continue;
        }
        ...
        if (attempt < maxRetries) {
          logger.warn(`[Attempt ${attempt}/${maxRetries}] Error: ${message}, retrying in 5s...`);
          await this.sleep(5000);
          continue;
        }
    ...
    logger.error('Transaction failed: Max retries reached');
    return { success: false };
  }
